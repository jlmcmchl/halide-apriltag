...
public:
    ...
    void schedule() {
        if (using_autoscheduler()) {
            ...
            input.set_estimates({{0, 1200}, {0, 800}});
...
// We will define a generator to auto-schedule.
class Greyscale : public Halide::Generator<Greyscale> {
public:
    ...
    void schedule() {
        if (using_autoscheduler()) {
            ...
            // set_estimates() method. set_estimates() takes in a list of
            // (min, extent) of the corresponding dimension as arguments.
            input.set_estimates({{0, 1200}, {0, 800}, {0, 3}});
...
HALIDE_REGISTER_GENERATOR(Greyscale, greyscale_gen)
