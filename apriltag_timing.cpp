...
void terminate_handler() {
  ...
  if (ex) {
    ...
    std::cerr << "std::terminate called (exception may have been thrown from "
                 "destructor or noexcept code)\n";
    std::abort();
  }
}
...
image_u8_t *image_u8_create_from_png(const char *path) {
  ...
  while (!feof(fp)) {
    ...
    uint32_t chunk_len =
        ((uint32_t)len_bytes[0] << 24) | ((uint32_t)len_bytes[1] << 16) |
        ((uint32_t)len_bytes[2] << 8) | ((uint32_t)len_bytes[3]);

    uint8_t chunk_type[4];
    if (fread(chunk_type, 1, 4, fp) != 4)
      break;
    ...
    if (strncmp((const char *)chunk_type, "IHDR", 4) == 0) {
      ...
      uint8_t *new_data =
          (uint8_t *)realloc(image_data, image_data_size + chunk_len);
      if (new_data == NULL) {
...
static DetectorRun run_detector(apriltag_family_t *tf, image_u8_t *im,
                                double decimate, double blur, int num_threads,
                                bool use_halide) {
  DetectorRun run;
  ...
  run.detect_time_ms =
      std::chrono::duration<double, std::milli>(detect_end - detect_start)
          .count();
...
// ai generated
// seems to work tho
static void report_diff(const DetectorRun &baseline,
                        const DetectorRun &halide) {
  ...
  for (const auto &entry : baseline_index) {
    ...
    for (size_t i = 0; i < count; i++) {
      ...
      double margin_diff =
          std::abs(base_det->decision_margin - halide_det->decision_margin);
      bool hamming_diff = base_det->hamming != halide_det->hamming;

      if (center_err > kCenterTolerance || max_corner_err > kCornerTolerance ||
          margin_diff > kMarginTolerance || hamming_diff) {
        printf("[COMPARE] Tag %d differs (center err %.3f px, max corner err "
               "%.3f px, margin diff %.3f, hamming %d vs %d).\n",
               id, center_err, max_corner_err, margin_diff, base_det->hamming,
               halide_det->hamming);
        any_diff = true;
      }
    }
  ...
  if (!any_diff) {
    printf("[COMPARE] Detected tags match within tolerances (center <= %.2f "
           "px, corners <= %.2f px).\n",
...
void print_usage(const char *program_name) {
  ...
  printf("Options:\n");
  printf("  -f, --family <name>    Tag family (tag36h11, tag25h9, tag16h5) "
         "[default: tag36h11]\n");
  printf("  -d, --decimate <val>   Decimate input image by this factor "
         "[default: 2.0]\n");
  printf("  -b, --blur <val>       Apply blur to input; negative sharpens "
         "[default: 0.0]\n");
  printf(
      "  -t, --threads <n>      Number of CPU threads to use [default: 1]\n");
  printf("  -r, --runs <n>         Repeat detections n times (averages "
         "timings) [default: 1]\n");
  printf("      --compare-halide   Run both baseline and Halide hybrid "
         "detectors\n");
...
int main(int argc, char *argv[]) {
  ...
  double img_load_time =
      std::chrono::duration<double, std::milli>(img_load_end - img_load_start)
          .count();
  ...
  if (run_halide && !halide_supported) {
    fprintf(stderr, "[WARN] Halide acceleration not available in this build. "
                    "Rebuild with USE_HALIDE=1.\n");
    if (halide_only) {
...
#ifdef APRILTAG_HAVE_HALIDE
  if (halide_valid) {
    print_run_summary("Halide hybrid (Halide threshold, UMich sparse)",
                      halide_run);
  }
#endif
  ...
  if (halide_valid) {
    ...
    printf("Copy to device:     mean %.3f ms, std %.3f ms over %zu run(s)\n",
           copy_in_stats.mean, copy_in_stats.stddev,
           threshold.copy_to_device_times_.size());
    printf("Pipeline:           mean %.3f ms, std %.3f ms over %zu run(s)\n",
           pipeline_stats.mean, pipeline_stats.stddev,
           threshold.pipeline_times_.size());
