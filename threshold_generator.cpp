  ...
  void schedule() {
    ...
    if (using_autoscheduler()) {
      ...
      output.split(x, x, xi, 64, TailStrategy::ShiftInwards)
          ...
          .gpu_blocks(x)
          .gpu_blocks(y)
          .split(xi, xi_serial_outer, xi, 8, TailStrategy::GuardWithIf)
          .gpu_threads(xi)
          .split(yi, yi_serial_outer, yi, 8, TailStrategy::GuardWithIf)
          .gpu_threads(yi);
      neigh_min.split(ty, ty, tyi, 8, TailStrategy::RoundUp)
          .unroll(tyi)
          .compute_at(output, x)
          .reorder(tyi, tx, ty)
          .split(tx, tx_serial_outer, tx, 8, TailStrategy::GuardWithIf)
          .gpu_threads(tx)
          .split(ty, ty_serial_outer, ty, 8, TailStrategy::GuardWithIf)
          .gpu_threads(ty);
      neigh_min.update(0)
      ...
      tile_min.update(0)
          ...
          .split(ty, ty_serial_outer, ty, 4, TailStrategy::GuardWithIf)
          .gpu_threads(ty);
      neigh_max.split(ty, ty, tyi, 8, TailStrategy::RoundUp)
          .unroll(tyi)
          .compute_at(output, x)
          .reorder(tyi, tx, ty)
          .split(tx, tx_serial_outer, tx, 8, TailStrategy::GuardWithIf)
      ...
      tile_max.update(0)
          ...
          .split(ty, ty_serial_outer, ty, 4, TailStrategy::GuardWithIf)
          .gpu_threads(ty);
      repeat_edge.split(_0, _0, _0i, 64, TailStrategy::ShiftInwards)
          ...
          .gpu_blocks(_0)
          .gpu_blocks(_1)
          .split(_0i, _0i_serial_outer, _0i, 64, TailStrategy::GuardWithIf)
          .gpu_threads(_0i);
    } else {
